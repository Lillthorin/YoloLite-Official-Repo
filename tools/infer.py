...
def draw_det(img_bgr: np.ndarray, boxes: np.ndarray, scores: np.ndarray, classes: np.ndarray, names: List[str]) -> np.ndarray:
    out = img_bgr.copy()
    for b, s, c in zip(boxes, scores, classes):
        x1, y1, x2, y2 = map(int, b.tolist())
        cls_name = names[int(c)] if 0 <= int(c) < len(names) else str(int(c))
        cv2.rectangle(out, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(out, f"{cls_name} {s:.2f}", (x1, max(0, y1 - 5)),
